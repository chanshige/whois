...
use Chanshige\DomainNormalizer;
 ...
 */
final class CcTLD extends AbstractCollection
{
    /** @var string[] */
    protected static array $data = [
        ...
        'tt', 'tv', 'tw', 'tz', 'ua', 'ug', 'uk', 'um', 'us', 'uy', 'uz', 'va', 'vc', 've',
        'vg', 'vi', 'vn', 'vu', 'wf', 'ws', 'ye', 'yt', 'za', 'zm', 'zw',
    ];

    public static function is(string $tld): bool
    {
        $domainName = (new DomainNormalizer())->normalize($tld);
        $tld = $domainName->tld();
        if ($tld === '') {
            $tld = $domainName->name();
        }
        ...
        return self::existsValue($tld);
    }

    public static function isIdn(string $tld): bool
    {
        return str_contains($tld, 'xn--');
    }
}
